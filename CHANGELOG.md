...
# Bug Fix
- Updated the `owner_resource` logic in `stg_shopify__metafield` to manage the new 'PRODUCTVARIANT' value for `owner_resource` produced by the `metafield` table. This ensures that the downstream model in `shopify__product_variant_metafields` brings in these values in the `dbt_shopify` transformation package.
