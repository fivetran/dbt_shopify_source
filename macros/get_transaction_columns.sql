{% macro get_transaction_columns() %}

{% set columns = [
    {"name": "id", "datatype": dbt.type_numeric()},
    {"name": "order_id", "datatype": dbt.type_numeric()},
    {"name": "refund_id", "datatype": dbt.type_numeric()},
    {"name": "amount", "datatype": dbt.type_numeric()},
    {"name": "created_at", "datatype": dbt.type_timestamp()},
    {"name": "processed_at", "datatype": dbt.type_timestamp()},
    {"name": "device_id", "datatype": dbt.type_numeric()},
    {"name": "gateway", "datatype": dbt.type_string()},
    {"name": "source_name", "datatype": dbt.type_string()},
    {"name": "message", "datatype": dbt.type_string()},
    {"name": "currency", "datatype": dbt.type_string()},
    {"name": "location_id", "datatype": dbt.type_numeric()},
    {"name": "parent_id", "datatype": dbt.type_numeric()},
    {"name": "payment_avs_result_code", "datatype": dbt.type_string()},
    {"name": "payment_credit_card_bin", "datatype": dbt.type_string()},
    {"name": "payment_cvv_result_code", "datatype": dbt.type_string()},
    {"name": "payment_credit_card_number", "datatype": dbt.type_string()},
    {"name": "payment_credit_card_company", "datatype": dbt.type_string()},
    {"name": "kind", "datatype": dbt.type_string()},
    {"name": "receipt", "datatype": dbt.type_string()},
    {"name": "currency_exchange_id", "datatype": dbt.type_numeric()},
    {"name": "currency_exchange_adjustment", "datatype": dbt.type_numeric()},
    {"name": "currency_exchange_original_amount", "datatype": dbt.type_numeric()},
    {"name": "currency_exchange_final_amount", "datatype": dbt.type_numeric()},
    {"name": "currency_exchange_currency", "datatype": dbt.type_string()},
    {"name": "error_code", "datatype": dbt.type_string()},
    {"name": "status", "datatype": dbt.type_string()},
    {"name": "test", "datatype": dbt.type_boolean()},
    {"name": "user_id", "datatype": dbt.type_numeric()},
    {"name": "_fivetran_synced", "datatype": dbt.type_timestamp()},
    {"name": "authorization_expires_at", "datatype": dbt.type_timestamp()}
] %}

{% if target.type in ('redshift','postgres') %}
 {{ columns.append({"name": "authorization", "datatype": dbt.type_string(), "quote": True, "alias": "authorization_code"}) }}
{% else %}
 {{ columns.append({"name": "authorization", "datatype": dbt.type_string(), "alias": "authorization_code"}) }}
{% endif %}

{{ return(columns) }}

{% endmacro %}


{% macro get_graphql_transaction_columns() %}

{% set columns = [
    {"name": "_fivetran_synced", "datatype": dbt.type_timestamp()},
    {"name": "account_number", "datatype": dbt.type_string()},
    {"name": "amount_rounding_set_pres_amount", "datatype": dbt.type_float()},
    {"name": "amount_rounding_set_pres_currency_code", "datatype": dbt.type_string()},
    {"name": "amount_rounding_set_shop_amount", "datatype": dbt.type_float()},
    {"name": "amount_rounding_set_shop_currency_code", "datatype": dbt.type_string()},
    {"name": "amount_set_pres_amount", "datatype": dbt.type_float()},
    {"name": "amount_set_pres_currency_code", "datatype": dbt.type_string()},
    {"name": "amount_set_shop_amount", "datatype": dbt.type_float()},
    {"name": "amount_set_shop_currency_code", "datatype": dbt.type_string()},
    {"name": "authorization_code", "datatype": dbt.type_string()},
    {"name": "authorization_expires_at", "datatype": dbt.type_timestamp()},
    {"name": "created_at", "datatype": dbt.type_timestamp()},
    {"name": "error_code", "datatype": dbt.type_string()},
    {"name": "extended_authorization_expires_at", "datatype": dbt.type_timestamp()},
    {"name": "formatted_gateway", "datatype": dbt.type_string()},
    {"name": "gateway", "datatype": dbt.type_string()},
    {"name": "id", "datatype": dbt.type_int()},
    {"name": "kind", "datatype": dbt.type_string()},
    {"name": "manual_payment_gateway", "datatype": dbt.type_boolean()},
    {"name": "manually_capturable", "datatype": dbt.type_boolean()},
    {"name": "maximum_refundable_amount", "datatype": dbt.type_float()},
    {"name": "maximum_refundable_currency_code", "datatype": dbt.type_string()},
    {"name": "multi_capturable", "datatype": dbt.type_boolean()},
    {"name": "order_id", "datatype": dbt.type_int()},
    {"name": "parent_id", "datatype": dbt.type_int()},
    {"name": "payment_avs_result_code", "datatype": dbt.type_string()},
    {"name": "payment_credit_card_bin", "datatype": dbt.type_string()},
    {"name": "payment_credit_card_company", "datatype": dbt.type_string()},
    {"name": "payment_credit_card_expiration_month", "datatype": dbt.type_int()},
    {"name": "payment_credit_card_expiration_year", "datatype": dbt.type_int()},
    {"name": "payment_credit_card_name", "datatype": dbt.type_string()},
    {"name": "payment_credit_card_number", "datatype": dbt.type_string()},
    {"name": "payment_credit_card_wallet", "datatype": dbt.type_string()},
    {"name": "payment_cvv_result_code", "datatype": dbt.type_string()},
    {"name": "payment_descriptor", "datatype": dbt.type_string()},
    {"name": "payment_icon_alt_text", "datatype": dbt.type_string()},
    {"name": "payment_icon_height", "datatype": dbt.type_int()},
    {"name": "payment_icon_id", "datatype": dbt.type_int()},
    {"name": "payment_icon_url", "datatype": dbt.type_string()},
    {"name": "payment_icon_width", "datatype": dbt.type_int()},
    {"name": "payment_id", "datatype": dbt.type_string()},
    {"name": "payment_method_name", "datatype": dbt.type_string()},
    {"name": "processed_at", "datatype": dbt.type_timestamp()},
    {"name": "receipt_json", "datatype": dbt.type_string()},
    {"name": "refund_id", "datatype": dbt.type_int()},
    {"name": "refund_set_acquirer_reference_number", "datatype": dbt.type_string()},
    {"name": "settlement_currency", "datatype": dbt.type_string()},
    {"name": "settlement_currency_rate", "datatype": dbt.type_float()},
    {"name": "staff_member_id", "datatype": dbt.type_int()},
    {"name": "standard_authorization_expires_at", "datatype": dbt.type_timestamp()},
    {"name": "status", "datatype": dbt.type_string()},
    {"name": "test", "datatype": dbt.type_boolean()},
    {"name": "total_unsettled_set_pres_amount", "datatype": dbt.type_float()},
    {"name": "total_unsettled_set_pres_currency_code", "datatype": dbt.type_string()},
    {"name": "total_unsettled_set_shop_amount", "datatype": dbt.type_float()},
    {"name": "total_unsettled_set_shop_currency_code", "datatype": dbt.type_string()}
] %}

{{ return(columns) }}

{% endmacro %}